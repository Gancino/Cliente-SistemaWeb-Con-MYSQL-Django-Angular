import { AfterViewInit, Component, OnInit, ViewChild } from '@angular/core';
import { MatPaginator } from '@angular/material/paginator';
import { MatSnackBar } from '@angular/material/snack-bar';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { Alertas } from '@data/constants';
import { IApiCongreso } from '@data/interfaces';
import { PrivateService } from '@data/services/api/private.service';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-show-con',
  templateUrl: './show-con.component.html',
  styleUrls: ['./show-con.component.scss']
})
export class ShowConComponent implements OnInit, AfterViewInit {

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;

  public loading!: boolean;
  public displayedColumns!: string[];
  public dataSource!: MatTableDataSource<any>;

  public ModalTitle!: string;
  public con!: any;
  public CongresoList!: IApiCongreso[];
  public AddEditConComp!: boolean;

  constructor(
    private service : PrivateService,
    private modal: NgbModal,
    private _snackBar: MatSnackBar
  ) { 
    this.CongresoList = [];
    this.displayedColumns = ['id_con', 'titulo_con', 'autor_con', 'anio_con', 'opciones'];
    this.loading = true;
    this.AddEditConComp = false;
  }

  ngOnInit(): void {
    this.refreshConList();
  }

  refreshConList(){
    this.service.getConList().subscribe(data => {
      this.loading=false;
      this.CongresoList=data;
      this.dataSource = new MatTableDataSource(this.CongresoList);
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
    });
  }

  ngAfterViewInit(): void {
    this.refreshConList();
  }

  estructModal(content: any){
    this.modal.open(content,{
      size: 'xl',
      centered: true,
      scrollable: true,
      backdrop: 'static',
      keyboard: false
    });
  }

  addClick(contenido: any){
    this.AddEditConComp = true;
    this.estructModal(contenido);
    this.con={
    id_con : 0,
    titulo_con : "",
    autor_con : "",
    anio_con : "",
    numero_con : "",
    }
    this.ModalTitle="Agregar un nuevo congreso";
  }

  closeClick(){
    this.modal.dismissAll();
    this.AddEditConComp = false;
    this.refreshConList();
  }

  editClick(contenido: any, item: any){
    this.AddEditConComp = true;
    this.estructModal(contenido);
    this.con=item;
    this.ModalTitle="Editar Congreso";
  }

  deleteClick(item: any){
    Swal.fire({
      title: Alertas.MSG_TITLE_DELETE, 
      text: Alertas.MSG_DELETE,
      width: '40%',
      padding: '1rem',
      backdrop: true,
      position: 'center',
      allowOutsideClick: true,
      stopKeydownPropagation: false,
      confirmButtonColor: '#3f51b5',
      icon: 'warning',
      showCancelButton: true,
      cancelButtonColor: '#d33',
      cancelButtonText: Alertas.MSG_CANCEL_DELETE,
      confirmButtonText: Alertas.MSG_CONFIRM_DELETE
    }).then((result) => {
      if (result.isConfirmed) {
        this.service.deleteCongreso(item.id_con).subscribe(data=>{
          Alertas.mostrarToast(data.toString(), this._snackBar);
          this.refreshConList();
        })
      }
    })
  }

  applyFilter(event: Event) {
    const filterValue = (event.target as HTMLInputElement).value;
    this.dataSource.filter = filterValue.trim().toLowerCase();
    //console.log(this.dataSource.filteredData);
  }

  recargarLista(){
    this.loading=true;
    setTimeout(() => {
      this.refreshConList();
      if(this.CongresoList.length>0){
        Alertas.mostrarToast(Alertas.MSG_LISTA_OBTENIDA, this._snackBar);
      }else{
        Alertas.mostrarToast(Alertas.MSG_LISTA_VACIA, this._snackBar);
      }
      this.refreshConList();
    }, 500);
  }

}
